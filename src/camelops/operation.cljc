(ns camelops.operation
  ...
  run. The advisor (CamelOpsAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Camelid Facility
  ...
  One run = one camelid-raising coordination operation (intake -> advise
  ...
  hands the decision to a human operator (rancher / herder /
  veterinarian). The approver resumes the pending request once a
  decision is made. `:flag-animal-health-concern` ALWAYS reaches
  escalation when the Governor is clean -- see
  `camelops.governor/always-escalate-ops`.
  ...
  (:require [camelops.advisor :as advisor]
...
(defn run-operation
  "Run one camelid-raising operation through the advisor -> governor ->
